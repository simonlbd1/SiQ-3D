...
def unet3():
    """
    Generate a 3D unet model with input size (120, 120, 16)
    Returns
    -------
    keras.Model
...
def _divide_img(img, unet_siz):
    """
    Divide a sub_images into multiple sub_images with the size used by the defined UNet
    Parameters
    ----------
    img : numpy.ndarray
...
class TrainingUNet3D:
    ...
    valid_label : numpy.array
        Raw images/labels
    bleach_revised_train_image : numpy.ndarray
    bleach_revised_valid_image : numpy.ndarray
        corrected photobleaching images
    ...
    def __init__(self, noise_level, folder_path, model):
        ...
        self.bleach_revised_train_image = None
        self.bleach_revised_valid_image = None
    ...
    def preprocess(self):
        """
        Correct photobleaching, normalize the images and divided them into small images for training the model
        """
        mean_0, background_0 = mean_background(self.train_image)
        self.bleach_revised_train_image = correct_bleaching(self.train_image, mean_0, background_0)
        self.bleach_revised_valid_image = correct_bleaching(self.valid_image, mean_0, background_0)
        print("Photobleaching was corrected")

        self.train_image_norm = _normalize_image(self.bleach_revised_train_image, self.noise_level)
        self.valid_image_norm = _normalize_image(self.bleach_revised_valid_image, self.noise_level)
    ...
    def save_losses(self):
        """
        Save train losses and validation losses of the 3D U-Net in csv file
        """
        train_losses = np.asarray(self.train_losses)
    ...
    def draw_losses(self):
        """
        Draw the train losses and validation losses.
        """
        plt.figure()
        plt.plot(self.train_losses, 'b', linewidth=2, label='Train loss')
        plt.plot(self.val_losses, 'r', linewidth=2, label='Validation loss')
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
